...
exports.handler = async function(context, event, callback) {
  try {
    ...
    if (checkin_response !== undefined && checkin_response !== null && checkin_response !== "") {
      status = checkin_response + "\n\n" + status;
    } else {
      //Only log if its a direct status call
      await logAction(name, sheets_service, context, 'status');
    }
